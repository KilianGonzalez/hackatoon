    ...
    <div>
      ...
      {error ? (
        ...
        <div className="bg-white rounded-xl border divide-y">
          {threads.map((tp) => {
            ...
            return (
              <Link
                key={tp.thread_id}
                href={`/app/company/messages/${tp.thread_id}`}
                className="flex items-start gap-4 p-4 hover:bg-gray-50 transition-colors"
              >
                <div className={`p-2 rounded-full ${hasUnread ? 'bg-orange-100' : 'bg-gray-100'}`}>
                  <MessageSquare className={`h-5 w-5 ${hasUnread ? 'text-orange-600' : 'text-gray-500'}`} />
                </div>
        ...
        <div className="bg-white rounded-xl border p-12 text-center">
          ...
          <p className="text-gray-500 mb-4">Contacta con los centros educativos</p>
          <Link
            href="/app/company/messages/new"
